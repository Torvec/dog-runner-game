...
export class Digger extends Enemy {
  constructor(level) {
    ...
    this.maxFrame = 7;
    this.image = new Image();
    this.image.src = "../assets/sprites/enemy/enemy_digger.png";
  }
}
...
export class HellSpinner extends Spinner {
  constructor(level) {
    super(level);
    this.position = {
      x: this.level.width + Math.random() * this.level.width * 0.5,
      y: (Math.random() + 0.1) * (this.level.height * 0.6),
    };
    this.speed = { x: Math.random() + 1, y: 0 };
  }
  update(deltaTime) {
...
export class BigBoss extends Enemy {
  constructor(level) {
    ...
    this.angleVelocity = Math.random() * 0.5;
    this.hitpoints = 50;
    this.image = new Image();
