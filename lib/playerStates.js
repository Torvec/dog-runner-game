...
export class Shooting extends PlayerState {
  ...
  handleInput(input) {
    if (this.level.player.frame.x >= 6 && this.level.player.onGround()) {
      this.level.player.setState("SITTING", 0);
    } else if (
      this.level.player.frame.x >= 6 &&
      (input.includes(controls.moveLeft) || input.includes(controls.moveRight))
    ) {
      this.level.player.setState("RUNNING", 1);
    }
  }
...
export class Hit extends PlayerState {
  ...
  handleInput() {
    if (this.level.player.frame.x >= 10 && this.level.player.onGround()) {
      this.level.player.setState("SITTING", 0);
      this.level.player.isInvincible = false;
    } else if (
      this.level.player.frame.x >= 10 &&
      !this.level.player.onGround()
    ) {
      this.level.player.setState("FALLING", 1);
      this.level.player.isInvincible = false;
