...
export class Sitting extends PlayerState {
  ...
  handleInput(input) {
    ...
    } else if (input.includes(controls.fire)) {
      this.level.player.setState("FIRING", 0);
    }
  }
}
...
export class Running extends PlayerState {
  ...
  handleInput(input) {
    ...
    if (input.includes(controls.sit)) {
      ...
      this.level.player.setState("FIRING", 1);
    }
  }
}
...
export class Jumping extends PlayerState {
  ...
  enter() {
    if (this.level.player.onGround()) this.level.player.vy -= 27;
    this.level.player.frameX = 0;
...
export class Rolling extends PlayerState {
  ...
  handleInput(input) {
    ...
    if (!input.includes(controls.roll) && this.level.player.onGround()) {
      this.level.player.setState("RUNNING", 1);
    } else if (
...
export class Firing extends PlayerState {
  constructor(level) {
    super("FIRING", level);
  }
  enter() {
    this.level.player.frameX = 0;
    this.level.player.maxFrame = 6;
    this.level.player.frameY = 7;
  }
  handleInput(input) {
    if (this.level.ammo > 0 && !this.level.spacePressed) {
      this.level.projectiles.push(
        new Projectile(
          this.level,
          this.level.player.x + this.level.player.width,
          this.level.player.y + this.level.player.height * 0.5
        )
      );
      this.level.ammo--;
    ...
    if (
      ...
      this.level.player.setState("JUMPING", 1);
    } else if (input.includes(controls.sit) || !input.includes(controls.fire)) {
      this.level.player.setState("SITTING", 0);
    }
  }
}
