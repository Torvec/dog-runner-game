...
export class Rolling extends PlayerState {
  ...
  handleInput(input) {
    ...
    if (!input.includes("Shift") && this.level.player.onGround()) {
      this.level.player.setState("RUNNING", 1);
    } else if (!input.includes("Shift") && !this.level.player.onGround()) {
      this.level.player.setState("FALLING", 1);
    } else if (
    ...
    } else if (input.includes("ArrowDown") && !this.level.player.onGround()) {
      this.level.player.setState("DIVING", 0);
    }
  }
}
...
export class Hit extends PlayerState {
  ...
  handleInput() {
    if (this.level.player.frameX >= 10 && this.level.player.onGround()) {
      this.level.player.setState("RUNNING", 1);
    } else if (this.level.player.frameX >= 10 && !this.level.player.onGround()) {
      this.level.player.setState("FALLING", 1);
    }
  }
}
