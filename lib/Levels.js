...
export class LevelBoss extends Level {
  ...
  update(deltaTime) {
    ...
    this.triggerWinCondition(this.bossEnemy && this.bossEnemy.hitpoints <= 0);
  }
  addEnemy() {
    if (!this.bossAdded && this.speed > 0 && Math.random() < 0.1) {
    ...
    this.enemies.push(new HellSpinner(this));
  }
}
