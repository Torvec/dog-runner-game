...
export class UI {
  ...
  displayLives(c) {
    ...
    this.livesImage.src = "../assets/sprites/ui/heart.png";
    c.drawImage(this.livesImage, 22, 25, 20, 20);
    c.save();
    c.shadowOffsetX = 0;
    c.shadowOffsetY = 0;
    c.fillStyle = "darkorange";
    c.fillRect(46, 25, 220, 20);
    c.restore();
    for (let i = 0; i < this.level.lives; i++) {
      c.fillRect(46 + 22 * i, 25, 20, 20);
    }
  }
  displayAmmo(c) {
    c.save();
  ...
  displayPower(c) {
    ...
    this.drawText(c, this.level.powerLevel + "%", this.font.size.base, 270, 84, this.font.align.right);
  }
  displayTimer(c) {
    this.drawText(
      c,
