...
class Scene {
  ...
  update() {
    this.currentState.changeState(this.game.input.keys);
  }
  draw(c) {
...
export class LevelFour extends GameLevel {
  constructor(game) {
    super(game);
...
export class HowToPlay extends Scene {
  constructor(game) {
    ...
    this.howToPlayText = {
      ...
      controls: {
        title: "Controls",
        text: "Press [ ] to Move Left, [ ] to Move Right,  [ ] to Jump, [ ] to Dive Bomb, [ ] to Fire Roll",
      },
      objective: {
        title: "Objective",
        text: "Get the highest Score within the time limit and with as many lives as possible",
      },
      playerActionText: "Press [ Enter ] to Continue",
    };
  }
  draw(c) {
    super.draw(c);
    ...
    c.font = "24px Impact";
    c.fillText(
      this.howToPlayText.playerActionText,
      this.game.width * 0.5,
      this.game.height - 50
    );
  }
